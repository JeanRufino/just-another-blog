...
export async function GET() {
  ...
  const presentes = data.map((p) => {
    const reserva = p.reservas?.[0] ?? null
    return {
      id: p.id,
      nome: p.nome,
      loja: p.loja,
...
export async function POST(req: NextRequest) {
  try {
    const { nome, loja, url, imagem_url } = await req.json()
    ...
    const { data, error } = await supabase
      .from('presentes')
      .insert({ nome, loja, url, imagem_url })
      .select()
      .single()
...
export async function PUT(req: NextRequest) {
  try {
    const { id, nome, loja, url, imagem_url } = await req.json()
    ...
    const { data, error } = await supabase
      .from('presentes')
      .update({ nome, loja, url, imagem_url })
      .eq('id', id)
      .select()
      .single()
